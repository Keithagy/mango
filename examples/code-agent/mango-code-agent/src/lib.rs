...
pub enum CodeAppError {
    ...
    TaskJoin(String),
    #[error("prompt ingress already running")]
...
pub struct PromptIngress {
    worker_id: WorkerId,
    commands: tokio::sync::mpsc::Sender<PromptIngressCommand>,
    receiver: Arc<Mutex<Option<tokio::sync::mpsc::Receiver<PromptIngressCommand>>>>,
    initial_prompt: Option<String>,
}

enum PromptIngressCommand {
    UserText(String),
    Interrupt,
    Close,
}
...
impl PromptIngress {
    pub fn new(prompt: impl Into<String>) -> Self {
        ...
        Self {
            ...
            initial_prompt: (!prompt.trim().is_empty()).then_some(prompt),
        }
    }
    ...
    pub async fn submit_text(&self, text: impl Into<String>) -> bool {
        self.commands
            .send(PromptIngressCommand::UserText(text.into()))
            .await
            .is_ok()
    }
    ...
    pub async fn interrupt(&self) -> bool {
        self.commands
            .send(PromptIngressCommand::Interrupt)
            .await
            .is_ok()
    }
    ...
    pub async fn close(&self) -> bool {
        self.commands
            .send(PromptIngressCommand::Close)
            .await
            .is_ok()
    }
}

impl Default for PromptIngress {
    fn default() -> Self {
        Self::interactive()
    }
}
    ...
    fn run(self, bus: CodeBus, session: CodeSession) -> Self::Run {
        Box::pin(async move {
            ...
            let mut commands = self
                ...
                .await
                .take()
                .ok_or(CodeAppError::IngressAlreadyRunning)?;
            ...
            if let Some(prompt) = self.initial_prompt {
                publish_prompt(bus, &session, prompt).await?;
            }

            while let Some(command) = commands.recv().await {
                match command {
                    PromptIngressCommand::UserText(text) => {
                        publish_prompt(bus, &session, text).await?;
                    }
                    PromptIngressCommand::Interrupt => {
                        publish::<CodeSchema, _>(
                            bus,
                            session_stream::<CodeSchema>(&session),
                            EventVisibility::Both,
                    ...
                    PromptIngressCommand::Close => break,
                }
            }
                            ...
                            publish::<CodeSchema, _>(
                                ...
                                EventPayload::Execution(ExecutionEvent::Control(
                                    ControlEvent::CancelRequested {
                                        session_id: self.session.session_id,
                                        run_id: Some(run_id),
                                        cause: Cancellation::SessionClosed,
                                    },
                                )),
                            )
                            .await?;
...
fn reset_coding_state(state: &mut CodingState) {
    state.current_run_id = None;
    state.next_sequence = 0;
    state.last_snapshot.clear();
}

async fn start_coding_run<B>(
    worker: &ClaudeCodingInference<B>,
    bus: &CodeBus,
    request_id: <CodeSchema as mango_core::agent::AgentSchemaIds>::InferenceRequestId,
    session_id: <CodeSchema as mango_core::agent::AgentSchemaIds>::SessionId,
    thread_id: <CodeSchema as mango_core::agent::AgentSchemaIds>::ThreadId,
    ...
    B: ClaudeBridgeLike,
{
    ...
    if let Err(error) = worker.bridge.send_user_text(directive.prompt).await {
        ...
        .await?;
        let mut state = worker.state.lock().await;
        reset_coding_state(&mut state);
        return Ok(state.session_closed);
    }

    Ok(false)
}

async fn cancel_coding_run<B>(
    worker: &ClaudeCodingInference<B>,
    bus: &CodeBus,
    run_id: Option<InferenceRunId>,
    cause: Cancellation<CodeSchema>,
) -> Result<bool, CodeAppError>
where
    B: ClaudeBridgeLike,
{
    let active_run = worker.state.lock().await.current_run_id;
    if run_id.is_some() && active_run != run_id {
        return Ok(false);
    }
    ...
    let mut state = worker.state.lock().await;
    reset_coding_state(&mut state);
    Ok(state.session_closed)
}

async fn mark_coding_session_closed<B>(
    worker: &ClaudeCodingInference<B>,
) -> Result<bool, CodeAppError>
where
    B: ClaudeBridgeLike,
{
    ...
    Ok(state.current_run_id.is_none())
}
    ...
    B: ClaudeBridgeLike,
{
    match payload {
        EventPayload::Execution(ExecutionEvent::Control(ControlEvent::Requested {
            ...
            return start_coding_run(worker, bus, request_id, session_id, thread_id, directive)
                .await;
        }
        EventPayload::Execution(ExecutionEvent::Control(ControlEvent::CancelRequested {
            ...
            cause,
            ..
        })) => return cancel_coding_run(worker, bus, run_id, cause).await,
        EventPayload::Interaction(InteractionEvent::SessionClosed { .. }) => {
            return mark_coding_session_closed(worker).await;
        }
        _ => {}
    }
...
async fn project_code_event(
    ...
    match payload {
        ...
        EventPayload::Interaction(InteractionEvent::SessionClosed { .. }) => return Ok(true),
        _ => {}
    }
...
impl SessionWorker<CodeSchema, CodeBus> for TerminalEgress {
    ...
    fn run(self, bus: CodeBus, session: CodeSession) -> Self::Run {
        Box::pin(async move {
            ...
            while let Some(event) = next_event(&mut events).await? {
                match event.payload {
                    EventPayload::Presentation(PresentationEvent::Outbound(outbound)) => {
                        match outbound {
                    ...
                    EventPayload::Interaction(InteractionEvent::SessionClosed { .. }) => break,
                    _ => {}
                }
            }
            Ok(())
            ...
            loop {
                tokio::select! {
                    Some(joined) = running_calls.join_next(), if !running_calls.is_empty() => {
                        ...
                        if session_closed && running_calls.is_empty() {
                            break;
                        }
                    }
                    maybe_event = next_event(&mut events), if !session_closed => {
                        let Some(event) = maybe_event? else {
                            session_closed = true;
                            continue;
                        };
...
mod tests {
    ...
    use mango_example_support::{ExampleBridge, ExampleSubstrate, ExampleSurface};
    ...
    async fn code_agent_streams_tool_events_and_answer() {
        ...
        let runtime = Arc::new(CodeAgentRuntime::new(
            ExampleSubstrate::new(CodeBus::new(256), PromptControl::new(session.clone())),
            ExampleSurface::new(
        ...
        let mut egress = runtime.surface().egress().subscribe();
