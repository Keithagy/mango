...
{
    let stream =
        BroadcastStream::new(state.runtime.egress().subscribe_ui()).filter_map(
            |event| match event {
                Ok(event) => {
...
where
    B: ClaudeBridgeLike,
{
    if state.runtime.ingress().submit_text(request.text).await {
...
where
    B: ClaudeBridgeLike,
{
    if state.runtime.ingress().interrupt().await {
        axum::http::StatusCode::ACCEPTED
    } else {
        axum::http::StatusCode::SERVICE_UNAVAILABLE
    }
...
mod tests {
    ...
    fn test_runtime(bridge: FakeClaudeBridge) -> Arc<ChatRuntime<FakeClaudeBridge>> {
    ...
    async fn chat_runtime_streams_user_and_assistant_events() {
        ...
        let runtime = test_runtime(bridge.clone());
        let mut ui_events = runtime.egress().subscribe_ui();
        ...
        assert!(runtime.ingress().submit_text("hello".to_string()).await);
    ...
    async fn chat_runtime_cancels_active_run() {
        ...
        assert!(runtime.ingress().interrupt().await);
