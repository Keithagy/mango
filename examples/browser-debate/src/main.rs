...
async fn main() -> Result<()> {
    ...
    let claude = ClaudeAgentBridge::spawn(
        ...
        .with_system_prompt_append(
            "You are the Claude side of a Mango browser debate example. Keep every answer brief, direct, and free of bullet lists.",
        ),
    )
    ...
    let runtime = DebateRuntime::new(
        ...
        ExampleSurface::new(
            ...
            ConcurrentBusWorkers::new(
                ...
                browser_debate::DebateProjector::new(session.clone()),
            ),
        ),
        ExampleBridge::new(
            ConcurrentBusWorkers::new(
                ...
                browser_debate::CodexDebater::new(session.clone(), codex),
            ),
            browser_debate::NoopToolsWorker::new(),
        ),
    );
    ...
    tokio::spawn({
        ...
        async move {
            if let Err(error) = runtime.run_session(session).await {
                tracing::error!("browser-debate runtime failed: {error}");
            }
        }
    });
    ...
    let bind_addr =
        std::env::var("MANGO_BROWSER_DEBATE_ADDR").unwrap_or_else(|_| "127.0.0.1:3002".to_string());
    let listener = tokio::net::TcpListener::bind(&bind_addr)
        .await
        .with_context(|| format!("failed to bind {bind_addr}"))?;

    info!("browser-debate listening on http://{bind_addr}");
